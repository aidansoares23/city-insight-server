...
//     // - name_asc (default)
//     // - livability_desc
//     // - safety_desc        (uses safetyScore 0–100)
//     // - rent_asc / rent_desc
//     // - reviews_desc       (reviewCount)
...
//     // 2) Fetch stats + metrics in parallel (doc reads)
//     const [statsSnaps, metricsSnaps] = await Promise.all([
//       Promise.all(baseCities.map((c) => db.collection("city_stats").doc(c.id).get())),
//       Promise.all(baseCities.map((c) => db.collection("city_metrics").doc(c.id).get())),
//     ]);
...
//     // 4) Optional search (by name/state/slug)
//     if (q) {
